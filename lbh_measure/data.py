...
class BagDataset(Dataset):
    ...
    @staticmethod
    def get_labels(box_points, roi_points):
        filter = np.in1d(roi_points, box_points)
        filter = filter.reshape(roi_points.shape)

        # Setting all labels 0 - background
        labels_one_hot = np.zeros((roi_points.shape[0]))
    ...
    def __getitem__(self, index):  # -> tuple(torch.tensor, torch.tensor):
        ...
        roi_pcd, ob_roi = self.crop_volume(pcd, parsed_data['roi'], 'roi')
        box_pcd, ob_box = self.crop_volume(pcd, parsed_data['box'], 'box')
        
        roi_points, roi_colors, roi_normals = self.get_np_points(roi_pcd)
        box_points, box_colors, box_normals = self.get_np_points(box_pcd)
        
#         threshold = 0.01
#         roi_pcd = roi_pcd.select_by_index(np.where(roi_points[:, 2]>threshold)[0])
#         box_pcd = box_pcd.select_by_index(np.where(box_points[:, 2]>threshold)[0])
        
#         roi_points, roi_colors, roi_normals = self.get_np_points(roi_pcd)
#         box_points, box_colors, box_normals = self.get_np_points(box_pcd)
        
        labels_one_hot = self.get_labels(box_points, roi_points)
        # The shape #points, 9 (3-xyz, 3-rgb, 3-normals)
        input_tensor = torch.zeros((roi_points.shape[0], 9))
        input_tensor[:, 0:3] = torch.tensor(roi_points)
        input_tensor[:, 3:6] = torch.tensor(roi_colors)
        if self.set_normals:
